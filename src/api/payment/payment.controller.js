...
module.exports = {
  ...
  pay: async (req, res, next) => {
    ...
    try {
      ...
      const PURCHASEHISTORYDATA = await global.models.GLOBAL.PURCHASEHISTORY.find({ Aid: Aid }).sort({ startDate: -1 });
      ...
      const newAmeninties = await global.models.GLOBAL.PURCHASEHISTORY(AmenintiesCreate);
      ...
      res
        ...
        .send(
          new APIResponse(
            ...
            messages.SUCCESS_PAYMENT, newAmeninties
          )
        );
    } catch (error) {
      console.log(error);
  ...
  confirmPayment: async (req, res, next) => {
    ...
    try {
      const userExist = await global.models.GLOBAL.PURCHASEHISTORY.findOne({ email: email });
      ...
      if (!Item) {
        ...
        console.log("zzzzzzzzzzzzzzzzzzzzzzzz", Item)
        Item = await global.models.GLOBAL.PURCHASEHISTORY.findByIdAndUpdate({ _id: id }, { isPaymentDone: true, PaymentId: PaymentId.client_secret });
        const data4createResponseObject = {
          req: req,
          result: 0,
        ...
        console.log("tttttttvvvvvvvvvvvvvvvvvvvvvvvvtttttt", Item)
        //  res.status(enums.HTTP_CODES.OK).json(utils.createResponseObject(data4createResponseObject));
      }
      ...
      const PURCHASEHISTORYDATA = await global.models.GLOBAL.PURCHASEHISTORY.find({ Aid: Aid }).sort({ startDate: -1 });
      console.log("paymentData", Aid);
      console.log("paymentData", PURCHASEHISTORYDATA);
