...
module.exports = exports = {
    ...
    handler: async (req, res) => {
        const { email, name, phone, Qsetid, ListofQA, currentProfession } = req.body;
        const { user } = req;
        ...
        // }
        if (!email || !name || !phone || !Qsetid || !ListofQA || !currentProfession) {
            const data4createResponseObject = {
                req: req,
                result: -1,
        ...
        try {
            const findAid = await global.models.GLOBAL.QUESTIONSET.findOne({ _id: Qsetid });
            const checkMenu = await global.models.GLOBAL.RESPONSE.find({ email: email, Qsetid: Qsetid });
            ...
            const newAmeninties = await global.models.GLOBAL.RESPONSE(AmenintiesCreate);
