...
module.exports = exports = {
  ...
  handler: async (req, res) => {
    try {
      ...
      const User = await global.models.GLOBAL.REGISTER.find(search)
        .sort({ createdAt: -1 })
        .populate({
          path: "uid",
          model: "admin",
      ...
      const Feedback = await global.models.GLOBAL.FEEDBACK.find(search)
        .sort({ createdAt: -1 })
        .populate({
          path: "ctid",
          model: "courseType",
      ...
      const data4createResponseObject = {
        ...
        payload: { User: User, Batch: batch, Feedback: Feedback },
        logPayload: false,
      };
      res
        .status(enums.HTTP_CODES.OK)
