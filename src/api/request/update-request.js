...
module.exports = exports = {
  ...
  handler: async (req, res) => {
    ...
    const { user } = req;
    if (!id || isAccept === undefined || !isAccept) {
    ...
    try {
      ...
      if (!property) {
        ...
        if (findRequest) {
          ...
          if (findRequest.part == "Banner" && findRequest.purpose == "Update") {
            let addBanner = {
              title: findRequest.title,
              image: findRequest.image,
              updatedAt: new Date(),
            };
            let findBanner = await global.models.GLOBAL.BANNER.findOneAndUpdate(
              { _id: findRequest.bid },
              addBanner,
              { new: true }
            );
          ...
          if (findRequest.part == "client" && findRequest.purpose == "Update") {
            let addBanner = {
              image: findRequest.image,
              updatedAt: new Date(),
            };
            let findClient = await global.models.GLOBAL.CLIENT.findOneAndUpdate(
